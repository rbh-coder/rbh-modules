...
trait RBH_ModuleFunctions
{
    ...
    private function RegisterReferenceVarIdList(string $itemsString) : void
    {
        if (!$this->IsValidStringList($itemsString)) return;
        foreach (explode(',', $itemsString) as $item) {
            if ($item != "") $this-> RegisterReferenceVarId($this->ReadPropertyInteger($item));
        }
    }
   ...
   private function HandleExpertSwitch(int $id, string $hideList, string $lockList ) : bool
   {
       ...
       else $status = GetValueInteger($id) == 0;
   ...
   private function DeleteProfileList (string $list) : void
   {
    ...
    {
        $this->SendDebug(__FUNCTION__, 'Set visibility of ' . $id . ' to ' . $hide, 0);
        if (!$this->IsValidId($id)) return;
    ...
    {
        $this->SendDebug(__FUNCTION__,'Set visibility of '.$item.' to ' . $status, 0);
